...
import sys, csv, json, time, math, queue, re, hashlib, uuid, threading
import copy
import logging
import faulthandler
...
from kinematics import (PALM_IDS, FRAME_IDS, LM_SMOOTH_DEFAULT, HOLD_MOTION_DEFAULT, HOLD_MOTION_PX,
                        VIEW3D_INTERVAL, VIEW3D_R, VIEW3D_ALPHA, APERTURE_MAX_CM, APERTURE_CUTOFF, APERTURE_BETA,
                        CYCLE_HYST, CYCLE_MIN_RANGE, AUTO_TRIAL_SEC, LM_XY, LM_Z, FINGERS, HANDS, HAND_KR,
                        JOINT_DEFS, FLEX_OF, JOINT_TRIPLE, TAM_JOINTS, ABD_KEYS, SEG_LABELS, SEG_COLORS,
                        CYCLE_JOINTS, FINGER_COLORS, LM_NAMES, OneEuro, LandmarkSmoother, AngleFilter,
                        ApertureFilter, angle3, joint_angles, palm_frame, thumb_abduction, _mono, ang_velocity,
                        count_cycles, sparc)

# ============================ 비디오 스레드 ============================
class VideoWorker(QThread):
    frame_processed = pyqtSignal(object, dict, float, int, dict, dict, dict)
    recording_finished = pyqtSignal(str, dict)
    recording_failed = pyqtSignal(str)
    failed = pyqtSignal(str)
    camera_waiting = pyqtSignal(str)
    ...
    def take_preview(self):
        try:
            return self.preview.get_nowait()
        except queue.Empty:
            return None

    def recording_status(self):
        recorder = self.recorder or self.saving_recorder
        return recorder.status() if recorder else {}

    def _publish_preview(self, packet):
        # Replace only the display image. All measurements still go to on_frame.
        try:
            self.preview.get_nowait()
        except queue.Empty:
            pass
        self.preview.put_nowait(packet)
    ...
    def _end_recording(self, error=None):
        if self.session_id is None:
            return
        session_id, self.session_id = self.session_id, None
        recorder, self.recorder = self.recorder, None
        self.saving_recorder = recorder
        camera_info = dict(self.camera_info)

        def finish():
            try:
                summary = (recorder.close() if recorder else
                           dict(complete=False, written_frames=0, error='원본 저장 시작 실패'))
            except Exception as exc:
                summary = dict(complete=False, written_frames=getattr(recorder, 'written', 0), error=str(exc))
            if error:
                summary.update(complete=False, error=error)
            summary['camera_info'] = camera_info
            self.recording_finished.emit(session_id, summary)

        # AVI closing/verification and queue draining must not stop the camera.
        self.finalizers = [thread for thread in self.finalizers if thread.is_alive()]
        thread = threading.Thread(target=finish, name='RecordingFinalizer', daemon=False)
        self.finalizers.append(thread)
        thread.start()

    def _recording_ready(self):
        if self.recorder is None:
            return True
        try:
            return self.recorder.ready()
            ...
            while self.running:
                ...
                if pipe is not None:
                    try:
                        ...
                        if now - last_camera_notice >= 3:
                            logging.warning('RealSense frame wait: %s', exc)
                            self.camera_waiting.emit('RealSense 영상 수신 대기 · 연결을 확인해주세요')
                            last_camera_notice = now
                        # A transient USB/SDK timeout is not a session time limit.
                        # Keep polling, process stop commands, and leave missing
                        # time as a real timestamp gap (never fabricate samples).
                        time.sleep(.02)
                        continue
                    capture_time, unix_time = time.perf_counter(), time.time()
                    cf0, df0 = original.get_color_frame(), original.get_depth_frame()
                    if not cf0:
                        if time.perf_counter() - last_camera_notice >= 3:
                            self.camera_waiting.emit('RealSense 컬러 프레임 수신 대기')
                            last_camera_notice = time.perf_counter()
                        continue
                    ...
                    if not ok:
                        if time.perf_counter() - last_camera_notice >= 3:
                            self.camera_waiting.emit('웹캠 영상 수신 대기 · 연결을 확인해주세요')
                            last_camera_notice = time.perf_counter()
                        time.sleep(0.01)
                        continue
                    capture_time, unix_time = time.perf_counter(), time.time()
                ...
                if self.recorder:
                    ...
                    try:
                        ...
                        # A storage failure ends this recording, not the camera.
                        self.recording_failed.emit(str(exc))
                        self._end_recording()
                ...
                self._publish_preview((output, angles_out, fps, n_hands, depths_out, hands3d, capture))
                self.frame_processed.emit(None, angles_out, fps, n_hands, depths_out, hands3d, capture)
        except Exception as exc:
            logging.exception('Camera worker stopped unexpectedly')
...
class PlotCanvas(FigureCanvas):
    def draw(self):
        # Export uses independent Agg figures. Skip a live plot paint while
        # exporting instead of blocking the GUI on Matplotlib's shared caches.
        if PLOT_LOCK.acquire(blocking=False):
            try:
                super().draw()
            finally:
                PLOT_LOCK.release()
...
class LiveAngleChart(PlotCanvas):
    ...
    def refresh(self, t):
        if not any(self.tbuf.values()) or (self._last is not None and t - self._last < .5):
            return
        self._last = t
...
#start { background:#059669; font-size:12px; min-height:24px; padding:5px 8px; }
#stop  { background:#dc2626; font-size:12px; min-height:24px; padding:5px 8px; }
#start:disabled, #stop:disabled { background:#2d3348; color:#64748b; }
#trial { background:#0284c7; font-size:12px; min-height:26px; padding:5px 8px; border:1px solid #38bdf8; }
...
class ClinicalApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self._build_ui()
        self.runtime_error.connect(self._runtime_error)
        ...
        self.worker.recording_failed.connect(self._recording_failed)
        self.worker.failed.connect(self._worker_failed)
        self.worker.camera_waiting.connect(self._camera_waiting)
    ...
    def _left_panel(self):
        ...
        f5 = QVBoxLayout(g5)
        f5.setSpacing(5)
        f5.setContentsMargins(10, 14, 10, 8)
        ...
        outer.addWidget(g5)
        monitor = QGroupBox('6. 실시간 기록 확인')
        grid = QGridLayout(monitor)
        grid.setContentsMargins(10, 14, 10, 8)
        grid.setHorizontalSpacing(6)
        grid.setVerticalSpacing(4)
        self.lbl_recording = QLabel('대기 · 세션을 시작하면 기록합니다')
        self.lbl_recording.setWordWrap(True)
        self.lbl_recording.setStyleSheet('color:#38bdf8; font-size:11px;')
        grid.addWidget(self.lbl_recording, 0, 0, 1, 3)
        self.lbl_record_counts = QLabel('저장 0 / 수신 0 · 대기 0장')
        self.lbl_record_counts.setStyleSheet('font-size:11px; color:#cbd5e1;')
        grid.addWidget(self.lbl_record_counts, 1, 0, 1, 3)
        self.lbl_latency = QLabel('처리 -- FPS · 화면 -- ms')
        self.lbl_latency.setStyleSheet('font-size:11px; color:#94a3b8;')
        grid.addWidget(self.lbl_latency, 2, 0, 1, 3)
        self.monitor_values = {}
        for col, hand in enumerate(HANDS, 1):
        ...
        for row, (key, label) in enumerate((('mp_aperture_mm', 'MP 원본 (mm)'),
            ...
            for col, hand in enumerate(HANDS, 1):
                ...
                value.setStyleSheet('font-size:11px; color:#e2e8f0;')
    ...
    def _trial_btn(self, starting):
        ...
        self.btn_trial.setText(
            f"{'▶' if starting else '⏹'}  {self.trial_idx}회차 · {tail}"
    ...
    def _recording_failed(self, message):
        logging.error('Recording error: %s', message)
        self._session_error = message
        self.toast(f'저장 오류: {message}')
        if self.session_on:
            self.stop_session()

    def _camera_waiting(self, message):
        self.paused = True
        self._trial_previous = None
        self.toast(message)

    def _runtime_error(self, message):
        # PyQt callbacks must not fall through to the default abort behaviour.
        # Preserve the existing session instead of silently continuing bad data.
        self.close_pending = False
        if getattr(self, '_handling_runtime_error', False):
            return
        self._handling_runtime_error = True
        self._session_error = f'화면 처리 오류: {message}'
        self.toast('처리 오류를 기록했습니다. 창은 유지하며 현재 데이터를 저장합니다.')
        if self.session_on and not self.finishing:
            self.stop_session()

    def _worker_stopped(self):
        self.btn_start.setEnabled(False)
        # The finalizer owns recording_finished even after the camera thread exits.
        # Do not export a fabricated empty summary while its real writer is draining.
        if self.session_on and not self.worker.finalizers:
    ...
    def stop_session(self):
        ...
        self.lbl_session.setText('● SAVING (원본·영상 저장 중)')
        self.lbl_session.setStyleSheet('color:#f59e0b; font-weight:bold;')
    ...
    def _recording_finished(self, session_id, summary):
        if session_id != self.session_id or not self.session_on:
            return
        self.capture_summary = summary
        logging.info('Recording finalized: session=%s received=%s written=%s error=%s',
                     session_id, summary.get('received_frames'), summary.get('written_frames'), summary.get('error'))
        ...
        if self.trial_on:
            try:
                ...
                logging.exception('Trial summary failed; preserving continuous session records')
                self.trial_on = False
                self._session_error = f'회차 요약 오류: {exc}'
    ...
    def _save_finished_session(self):
        if self.exporter is not None and self.exporter.isRunning():
            return
        try:
            snapshot = SessionExportData(self)
            self.exporter = SessionExporter(snapshot, self._stop_time or 0.0)
            self.exporter.finished.connect(self._export_finished)
            self.finishing = True
            self.btn_stop.setEnabled(False)
            self.btn_start.setEnabled(False)
            self._status('CSV·그래프 저장 마무리 중...', '#f59e0b')
            self.lbl_session.setText('● SAVING (CSV·그래프 저장 중)')
            self.exporter.start()
        except Exception as exc:
            self._complete_export(str(exc))

    def _export_finished(self):
        exporter = self.exporter
        self.exporter = None
        self._complete_export(exporter.error)
        exporter.deleteLater()

    def _complete_export(self, error):
        self.finishing = False
        ...
        if error:
            ...
            self.lbl_session.setText('● 저장 실패 (재시도 가능)')
            self.lbl_session.setStyleSheet('color:#ef4444; font-weight:bold;')
            self.toast(f'저장 실패: {error}. 저장 공간·폴더를 확인한 후 재시도해주세요.')
            return
        self.unsaved = False
        self._handling_runtime_error = False
        ...
        self.lbl_session.setStyleSheet(f'color:{"#f59e0b" if error else "#10b981"}; font-weight:bold;')
        if error:
    ...
    def save_session(self, duration):
        ...
        meta = {
            ...
            'source_sha256': {name: hashlib.sha256((Path(__file__).parent / name).read_bytes()).hexdigest()
                              for name in ('Mirror_therapy.py', 'mirror_capture.py', 'mirror_mjpeg.py')},
    ...
    def export_plot(self, path, title):
        fig = Figure(figsize=(11, 9), dpi=150)
        FigureCanvasAgg(fig)
        axes = fig.subplots(3, 1, sharex=True, gridspec_kw={'height_ratios': [3, 3, 2]})
    ...
    def export_joint_plot(self, path, title):
        ...
        nrow = len(FINGERS) + 1
        fig = Figure(figsize=(13, 15), dpi=150)
        FigureCanvasAgg(fig)
        axes = fig.subplots(nrow, 2, sharex=True)
        fig.patch.set_facecolor('#ffffff')
    ...
    def _render_preview(self, packet, now):
        ...
        for hand in HANDS:
            ...
            measurement = capture.get('measurements', {}).get(hand, {})
    ...
    def _refresh_record_monitor(self, now):
        status = self.worker.recording_status()
        written = status.get('written', self.capture_summary.get('written_frames', 0))
        received = status.get('received', self.capture_summary.get('received_frames', 0))
        pending = status.get('pending', 0)
        self.lbl_record_counts.setText(f'저장 {written} / 수신 {received} · 대기 {pending}장')
        error = (self._session_error or self._export_error or
                 status.get('error') or self.capture_summary.get('error'))
        if error:
            text, color = '부분 저장 · 오류 확인 필요', '#ef4444'
        elif self.finishing or self.unsaved:
            text, color = '저장 마무리 중 · 카메라 표시 유지', '#f59e0b'
        elif self.session_on:
            if status.get('backpressure'):
                text, color = '기록 중 · 저장 속도에 맞춰 촬영 조절', '#f59e0b'
            else:
                text, color = '● 기록 중 · 원본 + MediaPipe', '#10b981'
        elif self.capture_summary:
            text, color = '저장 완료 · 원본 + MediaPipe', '#10b981'
        else:
            text, color = '대기 · 세션을 시작하면 기록합니다', '#38bdf8'
        self.lbl_recording.setText(text)
        self.lbl_recording.setStyleSheet(f'color:{color}; font-size:11px;')
        if self._last_capture is not None and now - self._last_capture > 1:
            self.lbl_latency.setText(f'새 프레임 대기 · {now-self._last_capture:.1f}초')
            for value in self.monitor_values.values():
                value.setText('--')

    def _tick(self):
        now = time.perf_counter()
        packet = self.worker.take_preview()
        if packet is not None:
            self._render_preview(packet, now)
...
class SessionExportData:
    """Frozen GUI values and completed records; the export thread never reads Qt widgets."""
    _f = staticmethod(ClinicalApp._f)
    _trial_metrics = staticmethod(ClinicalApp._trial_metrics)
    save_session = ClinicalApp.save_session
    _export_comparison = ClinicalApp._export_comparison
    export_plot = ClinicalApp.export_plot
    export_joint_plot = ClinicalApp.export_joint_plot

    def __init__(self, app):
        for name in ('session_id', 'folder', '_session_error'):
            setattr(self, name, getattr(app, name))
        # Completed lists are no longer appended to. Starting another session is
        # disabled until export ends; copy trial dicts because metrics are refreshed.
        self.records = list(app.records)
        self.frame_log = list(app.frame_log)
        self.trials = copy.deepcopy(app.trials)
        self.capture_summary = copy.deepcopy(app.capture_summary)
        self.worker = SimpleNamespace(source_name=app.worker.source_name)
        groups = {'isChecked': ('chk_filter', 'chk_smooth', 'chk_mirror', 'chk_hold',
                                 'rb_healthy', 'rb_patient'),
                  'value': ('spin_age', 'spin_fma', 'spin_palm', 'spin_auto'),
                  'text': ('txt_name',),
                  'currentText': ('cb_gender', 'cb_brs', 'cb_affected')}
        for getter, names in groups.items():
            for name in names:
                value = getattr(getattr(app, name), getter)()
                setattr(self, name, SimpleNamespace(**{getter: lambda value=value: value}))


class SessionExporter(QThread):
    def __init__(self, data, duration):
        super().__init__()
        self.data, self.duration, self.error = data, duration, None

    def run(self):
        try:
            with PLOT_LOCK:
                self.data.save_session(self.duration)
        except Exception as exc:
...
def main():
    ...
    logging.basicConfig(filename=str(log_dir / f'runtime_{stamp}.log'),
                        level=logging.INFO, encoding='utf-8',
                        format='%(asctime)s %(levelname)s %(message)s')
    # Keep this descriptor alive until QApplication exits; native-extension
    # crashes bypass Python exception handlers but may still leave a traceback.
    crash_log = (log_dir / f'crash_{stamp}.log').open('a', encoding='utf-8')
    faulthandler.enable(file=crash_log, all_threads=True)
    logging.info('Application started: %s | Python %s', __file__, sys.version)
    ...
    def report_exception(kind, value, tb):
        logging.error('Unhandled Qt/Python callback exception', exc_info=(kind, value, tb))
        win.runtime_error.emit(f'{kind.__name__}: {value}')

    sys.excepthook = report_exception
    win.showFullScreen()
    try:
        exit_code = app.exec()
        logging.info('Application event loop exited: %s', exit_code)
    finally:
