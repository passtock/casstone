# -*- coding: utf-8 -*-
"""
Generate '연구계획서20260911_통합본.docx' & Update '연구계획서20260911.docx'
RGB-D 운동학 정보를 활용한 양손 손 과제 VLM 평가 파일럿 연구 계획서 (최종 통합본 - 2026년 9월 11일)
Author: 한동대학교 기계제어공학 · Human Robotics Lab 22000561 이재용
Merged Strengths:
  1. 안티그래비티의 실존하는 5대 모델 라인업 (Qwen3-VL 2종 + LLaVA-NeXT-Video-7B, Qwen2.5-VL-32B, LLaVA-OneVision-7B)
  2. 단일 PC RTX 5090 (32GB VRAM) 100% 로컬 온프레미스 완벽 구동 검증표 (가중치+비전토큰+KV캐시)
  3. 지피티의 친절한 사전 개념 풀이 (VLM, RGB-D, 운동학 수치, 14프레임, A0~A4 명명 배경, null과 0의 구분 등)
  4. 양측의 선행 베이스 연구 9편 1:1 대응표 및 부록 D 상세 학술 분석 결합
  5. 격식 있는 네이비/슬레이트 테마, 정교한 표 여백, 콜아웃 박스, 코드 블록 서식 완비
"""
...
def create_integrated_proposal():
    doc = Document()
    ...
    print("Synthesizing Best-of-Both-Worlds Document...")
    ...
    p_sub = add_para(doc, "연구계획서 개정 통합본 (2026년 9월 11일)", 
    ...
    # =========================================================================
    # SECTION 0: 연구 개요 및 사전 개념 체계 (친절한 풀이 + 엄밀한 구조화 결합)
    # =========================================================================
    ...
    add_para(doc, 
        ...
        "3. 운동학 수치(Kinematic Features): 힘이나 토크가 아닌, 손 관절 기준점 사이의 거리, 최대 벌림 간격에 도달한 시간, 유지 중 떨림 표준편차 등 순수 움직임의 기하학과 시간을 요약한 물리적 측정값이다. (AI 모델의 학습 가중치 파라미터와 전혀 다름).\n"
        "4. 파지(Grasp): 물체를 손으로 쥐는 형태를 뜻하며, 본 연구에서는 '목표 형태 만들기(형성)' → '2초 유지' → '손가락 능동 펴기(해제)'의 3단계를 평가한다.\n"
        "5. 14프레임 시간 균일 샘플링: 수백 프레임의 전체 동영상을 그대로 넣으면 모델의 연산 한도를 초과하므로, 동작 전개 과정을 대표하는 14장의 이미지를 수학적 규칙으로 공정하게 추출하여 VLM에 제공한다."
    )
    ...
    add_callout(doc, "핵심 임상 배경 및 연구 가설의 요약 (Why this research?)", [
        ...
        "2. VLM의 기회와 치명적 한계 (Opportunity & Visual Hallucination): 최신 VLM은 일상 행동 인식(77.5%)에는 우수하나, 미세 손가락 움직임 정량화 및 파지 접촉 여부 판단에서 '물체 근접을 실제 파지로 오인'하거나 '존재하지 않는 움직임을 생성'하는 심각한 시각적 환각(Visual Hallucination)이 발생함 (Li et al., 2026 선행연구 핵심 입증).",
        "3. 해결 가설 (Proposed Solution): 동결된(Frozen Zero-shot) 오픈소스 VLM에 2D 영상뿐 아니라, 인텔 RealSense D455 RGB-D 깊이 센서에서 정량 추출한 4대 3D 운동학 수치(F1~F4)와 건강인 대조군 정상치 규준을 프롬프트로 주입하면, 시각적 모호성을 극복하고 전문가 평가 일치도를 획기적으로 개선할 수 있을 것이다.",
        "4. 100% 로컬 온프레미스 인프라: 환자의 민감한 의료 비디오가 외부 클라우드로 유출되는 위험을 원천 차단하고, 연구실 단일 PC(NVIDIA RTX 5090, 32GB VRAM)에서 5개 모델 전체를 완전 오프라인으로 구동함."
    ])
    ...
    add_para(doc, 
        ...
        "A1~A4는 동일한 VLM의 입력 조건이고, A0의 마지막 글자는 알파벳 O가 아니라 숫자 0이며 별도로 지도학습하는 고전 머신러닝 비교 모델이다. "
    ...
    summary_rows = [
        ...
        ["VLM 하드웨어 및 모델\n(외부 API 완전 배제)", "단일 PC 워크스테이션: NVIDIA GeForce RTX 5090 (32GB GDDR7 VRAM)\n[최신 오픈소스 모델군 2종]\n- Qwen/Qwen3-VL-8B-Instruct (BF16, ~20.5 GB 점유)\n- Qwen/Qwen3-VL-30B-A3B-Instruct (AWQ 4-bit, ~22.5 GB 점유)\n[선행연구 기준 재현 모델군 3종]\n- lmms-lab/LLaVA-NeXT-Video-7B (BF16, ~19.0 GB 점유)\n- Qwen/Qwen2.5-VL-32B-Instruct (AWQ 4-bit, ~24.5 GB 점유)\n- lmms-lab/llava-onevision-qwen2-7b-ov (BF16, ~19.5 GB 점유)", "외부 API 호출 없음 (의료 데이터 프라이버시 완벽 보호).\n5개 모델 전체가 RTX 5090 (32GB) 1대에서 100% 로컬 구동 (모두 25GB 이하 점유)."],
        ...
        ["전체 실행 규모", "핵심 조건(A1~A4) 480시행 × 4조건 × 5모델 = 9,600회\n+ 피처 제거(D1~D4) 480시행 × 4조건 = 1,920회\n+ 반복성 검증 60시행 × 4조건 × 5모델 × 2회 = 2,400회\n= 총 13,920회 요청.", "개발 단계 4명 자료로 메모리 및 지연시간 검증 후 배치 실행."]
    ]
    add_styled_table(doc, summary_headers, summary_rows, [2.5, 9.4, 4.0])

    # =========================================================================
    # SECTION 1: 연구 배경 및 필요성 (베이스 선행연구 심층 고찰)
    # =========================================================================
    ...
    add_para(doc, 
        ...
        "구분하지 못하는 치명적 오분류 한계를 함께 보고하였다."
    )

    add_h2(doc, "1.2 순수 영상 VLM의 기회와 치명적 병목: 시각적 환각 (Visual Hallucination)")
    add_para(doc, 
    ...
    add_para(doc, 
        ...
        "손가락의 미세 동작 정량화와 FMA 운동 손상 점수 예측에서는 무작위 추정 수준에 머물렀다. "
        "특히 가장 심각한 문제는 '시각적 환각(Visual Hallucination)'이었다. 환자의 손이 물체에 단순히 근접해 지나치기만 했는데도 "
        "실제 물체를 잡은 것으로 잘못 판독하거나, 환자가 시도조차 하지 않은 움직임을 수행했다고 허구로 보고하는 오류가 빈번히 발생하였다. "
        "이는 2D 영상 프레임만으로는 깊이 방향의 접촉 여부, 손가락 벌림 간격(Aperture), 미세 떨림을 모델이 공간적으로 해석할 수 없기 때문이다. "
        "따라서 순수 영상 모델에 물리적 3D 공간 수치를 주입하여 이러한 환각을 제어할 공학적 방법론의 개발이 시급하다."
    )
    ...
    add_para(doc, 
        "이러한 3D 운동학을 마커리스(Markerless) 센서로 획득할 가능성은 Amprimo 등(2024, Biomedical Signal Processing and Control)에 의해 검증되었다. "
        "그들은 Google MediaPipe 손 추적기에 RGB-D 센서(Azure Kinect / RealSense)의 깊이 정보를 결합한 GMH-D 프레임워크를 개발하고, "
        "120 fps OptiTrack 광학 모션캡처와 동시 비교하여 손끝 간격 및 거리 변화 시계열이 모션캡처와 높은 일치도를 보임을 입증하였다."
    )
    add_para(doc, 
        ...
        "그러나 이들 연구는 보행이나 전신 운동에 한정되어 있으며, 손가락의 복잡한 폐색(Occlusion)과 정밀 파지 형태가 지배하는 "
        "뇌졸중 손 과제에서 '재학습(Fine-tuning) 없는 동결 VLM'에 영상과 3D 운동학 수치를 동시 주입했을 때의 정량적 오차 감소 효과는 "
        "국제적으로 전혀 검증된 바 없다. 이것이 본 연구가 해결하고자 하는 핵심 연구 공백(Research Gap)이다."
    )

    add_h2(doc, "1.4 선행 베이스 연구가 남긴 질문과 본 연구의 대응 체계")
    base_headers = ["선행 베이스 연구", "선행연구에서 이미 확인된 사실", "현재 조건에서 여전히 모르는 한계점", "본 연구계획서의 공학적 대응"]
    base_rows = [
        ["Li et al. (2026)\nPLOS Digit Health", "15개 VLM이 거친 활동은 인식하나 미세 동작 정량화 및 채점에서 환각(오판) 발생 입증.", "새로운 VLM에서도 동일한 오류가 남는가? 물리적 센서 수치를 주입하면 이 오판이 억제되는가?", "Qwen2.5-VL-32B 및 최신 Qwen3-VL 확정, 14프레임 영상에 D455 3D 수치 결합 효과(A2−A1) 직접 검증."],
        ...
        ["Amprimo et al. (2024)\nBSPC", "MediaPipe에 RGB-D 깊이를 결합하여 120fps 광학 모션캡처 수준의 거리 시계열 추출 가능.", "물체를 쥐는 파지 상황의 손가락 가림(Occlusion) 환경에서도 안정적 피처 추출이 가능한가?", "정적 5mm MAE 벤치마크, 80% 유효율 기준, 가림 발생 시 null 플래그 품질 관리 규칙 확립."],
        ...
        ["Tang et al. (2025)\nWang et al. (2024)", "수치를 LLM/VLM에 주입하여 임상 지식을 보강하는 프롬프팅 발상의 유효성 입증.", "수치만 주는 것(A4)과 영상을 함께 주는 것(A3), 머신러닝(A0) 중 임상 평가 최적 구성은?", "입력 블록을 엄격히 분리한 A1~A4 체계 및 LOSO-CV 다항 로지스틱 회귀 A0 기준군 동시 구축."]
    ]
    ...
    add_para(doc, 
        ...
        "동일한 뇌졸중 환자의 손 과제에 대해 '14프레임 RGB 영상에 3D 운동학 수치를 추가한 조건(A2)'이 '순수 영상만 제공한 조건(A1)'보다 "
    ...
    comp_rows = [
        ...
        ["보조 비교 3", "A3 − A1", "순수 영상 대비 운동학 수치와 건강인 참조를 모두 결합한 전체 복합 효과", "초기 계획서의 복합 정보 주입 효과 재확인 (보조적 확인)"],
        ["보조 비교 4", "A3 − A0", "지도학습된 전통 머신러닝 수치 모델(A0) 대비 Zero-shot 거대 VLM(A3)의 실용적 성능 차이", "환자 데이터로 훈련한 모델 vs 재학습 없는 파운데이션 모델의 실용적 비교"],
    ...
    add_para(doc, 
        ...
        "환자의 민감한 의료 영상 데이터가 외부 클라우드나 네트워크로 유출될 위험이 원천적으로 차단된다."
    )
    ...
    add_para(doc, 
        "연구자가 비디오를 육안으로 보며 자의적으로 유지 구간을 슬라이싱하는 주관적 편향(Cherry-picking)을 원천 차단하기 위해, "
    ...
    add_para(doc, 
        ...
        "45° 부감 앵글에서 물체 및 손바닥에 의한 카메라 가림(Occlusion)이 가장 적어 RealSense Depth의 결측(Hole) 없이 가장 안정적으로 측정되기 때문이다. "
        ...
        "대신 이들 전체 관절 각도와 속도 데이터는 고전 머신러닝 기준 모델(A0)의 학습 피처 및 확장 소거 실험(A2_ext)의 비교 검증 변수로 전량 투입하여 학술적 깊이를 극대화한다."
    )
    ...
    add_h2(doc, "5.2 균일 14프레임 시간 샘플링 규칙")
    ...
    # =========================================================================
    # SECTION 6: VLM 모델 구성, 프롬프트 및 단일 PC RTX 5090 환경 (Antigravity 5대 모델 라인업)
    # =========================================================================
    add_h1(doc, "6. VLM 모델 구성, 프롬프트 및 단일 PC RTX 5090 환경")
    
    add_h2(doc, "6.1 단일 PC RTX 5090 (32GB VRAM) 하드웨어 환경 및 확정 모델 라인업")
    add_para(doc, 
        "본 연구는 외부 상용 API(Gemini, OpenAI 등)를 일체 사용하지 않으며, "
        "연구실 단일 워크스테이션(NVIDIA GeForce RTX 5090 32GB GDDR7, Blackwell 아키텍처)에서 100% 오프라인으로 구동된다. "
        "모델군은 최신 고성능 모델 2종과 선행연구 기준 재현 모델 3종으로 엄격히 구성되었다. "
        "모든 모델은 14개 프레임 비전 토큰과 텍스트 프롬프트를 포함하여 32GB VRAM 내에서 완벽하게 구동됨을 사전 검증하였다."
    )
    
    vram_headers = ["모델 분류", "공식 모델 식별자 (HF Model ID)", "가중치 정밀도", "가중치 VRAM", "비전+KV캐시", "총 VRAM 점유량", "32GB 안전성"]
    vram_rows = [
        ["최신 모델 1\n(주 로컬 모델)", "Qwen/Qwen3-VL-8B-Instruct", "BF16 (16-bit 무손실)", "약 16.0 GB", "약 4.5 GB", "약 20.5 GB", "여유 11.5 GB (36% 마진)\n초고속 추론, OOM 제로"],
        ["최신 모델 2\n(고성능 MoE)", "Qwen/Qwen3-VL-30B-A3B-Instruct", "AWQ (4-bit 양자화)", "약 16.5 GB", "약 6.0 GB", "약 22.5 GB", "여유 9.5 GB (30% 마진)\nMoE 고성능 단일 GPU 완벽 구동"],
        ["선행 재현 1\n(비디오 특화)", "lmms-lab/LLaVA-NeXT-Video-7B", "BF16 (16-bit 무손실)", "약 14.5 GB", "약 4.5 GB", "약 19.0 GB", "여유 13.0 GB (41% 마진)\n비디오 시계열 특화 벤치마크"],
        ["선행 재현 2\n(NYU 베이스 SOTA)", "Qwen/Qwen2.5-VL-32B-Instruct", "AWQ (4-bit 양자화)", "약 18.5 GB", "약 6.0 GB", "약 24.5 GB", "여유 7.5 GB (23% 마진)\nLi et al.(2026) 32B SOTA 직접 재현"],
        ["선행 재현 3\n(멀티모달 표준)", "lmms-lab/llava-onevision-qwen2-7b-ov", "BF16 (16-bit 무손실)", "약 15.0 GB", "약 4.5 GB", "약 19.5 GB", "여유 12.5 GB (39% 마진)\nAnyRes 멀티프레임 표준 벤치마크"]
    ]
    add_styled_table(doc, vram_headers, vram_rows, [2.2, 4.2, 2.2, 1.8, 1.8, 2.0, 1.7])

    add_h2(doc, "6.2 공통 추론 설정 및 이미지 처리")
    add_para(doc, 
        "프롬프트는 지침과 입력 자료를 담은 요청문이며, 각 모델의 공식 입력 변환기(processor)와 대화 형식(chat template)을 사용한다. "
        "비교의 엄격성을 위해 공통 설정으로 BF16 정밀도, 배치 크기 1, 탐욕적 생성(do_sample=False, num_beams=1)을 적용한다. "
        "생성 토큰 상한은 max_new_tokens=1024, repetition_penalty=1.0으로 고정하며, "
        "메모리 절약을 위해 PyTorch의 스케일 점곱 주의 연산(SDPA, attn_implementation=sdpa)을 공통 활성화한다. "
        "외부 입력은 모든 모델에 896×504 크기의 동일한 14장 이미지를 시간순으로 제공한다."
    )

    add_h2(doc, "6.3 전체 추론 실행량 계획")
    add_para(doc, 
        "5개 로컬 모델에 대한 전체 실행량은 다음과 같이 산정된다. "
        "1) 핵심 실행: 480개 환자 시행 × 4개 조건(A1~A4) × 5개 모델 = 9,600회. "
        "2) 피처 소거(D1~D4): 주 모델(Qwen3-VL-8B) 480시행 × 4조건 = 1,920회. "
        "3) 반복성 검증: 환자별 첫 시행(60시행) × 4조건 × 5모델 × 2회 추가 = 2,400회. "
        "총 합계는 13,920회 요청이며, RTX 5090의 고속 배치 추론 능력을 바탕으로 약 18~22시간 내에 전체 로컬 추론이 완결된다."
    )
    ...
    add_para(doc, 
        ...
        "외부 파이썬 코드가 위 표의 규칙에 따라 엄격하게 0, 1, 2점으로 결정론적 변환한다. 이를 통해 모델의 점수 왜곡 및 환각을 차단한다."
    )
    json_example = (
        "{\n"
        '  "left_hand": {\n'
    ...
    add_para(doc, 
        ...
        "또한 거대 VLM의 실용적 도입 타당성을 확인하기 위해 동일 수치를 사용하는 다항 로지스틱 회귀 기준 모델 A0(LOSO-CV 15-fold)와의 "
    ...
    int_rows = [
        ...
        ["평균 오차 감소와 유효 판독률(Coverage) 하락 동반", "판독하기 쉬운 쉬운 시행만 남아 오차가 낮아 보이는 착시(Selection bias) 가능성. 최대 오차(2점)를 부여한 보수적 민감도 분석과 함께 해석."],
    ...
    add_para(doc, 
        ...
        "단독 무인 평가 시스템의 완성을 주장하지 않으며, 치료사의 비디오 판독 부담을 경감하기 위한 "
    ...
    chk_rows = [
        ...
        ["카메라 정적 벤치마크", "70cm 거리에서 20·40·60·80mm 기준 블록 간격 오차 MAE ≤ 5 mm", "개발 건강인 4명 대상 3회 반복 측정"],
    ...
    # APPENDICES
    # =========================================================================
    add_h1(doc, "부록 A. 로컬 VLM 5종 공식 식별자 및 RTX 5090 로딩 가이드")
    add_para(doc, 
        "HuggingFace 공식 저장소의 공개 모델 식별자 및 단일 RTX 5090 (32GB) 환경의 권장 로딩 구성은 다음과 같다."
    )
    app_a_headers = ["모델 역할", "HuggingFace Model ID", "가중치 형식 및 패키지", "VRAM 사용량", "로딩 파이프라인 코드 예시"]
    app_a_rows = [
        ["최신 주 로컬 모델", "Qwen/Qwen3-VL-8B-Instruct", "BF16 / transformers (v4.49+)", "약 20.5 GB", "AutoModelForVision2Seq.from_pretrained(..., torch_dtype=torch.bfloat16, device_map='cuda')"],
        ["최신 고성능 MoE", "Qwen/Qwen3-VL-30B-A3B-Instruct", "AWQ 4-bit / autoawq, transformers", "약 22.5 GB", "AutoModelForVision2Seq.from_pretrained(..., device_map='cuda')"],
        ["선행 비디오 재현", "lmms-lab/LLaVA-NeXT-Video-7B", "BF16 / transformers", "약 19.0 GB", "LlavaNextVideoForConditionalGeneration.from_pretrained(..., torch_dtype=torch.bfloat16).cuda()"],
        ["선행 SOTA 재현", "Qwen/Qwen2.5-VL-32B-Instruct", "AWQ 4-bit / autoawq, transformers", "약 24.5 GB", "AutoModelForVision2Seq.from_pretrained(..., device_map='cuda')"],
        ["선행 멀티모달 재현", "lmms-lab/llava-onevision-qwen2-7b-ov", "BF16 / transformers", "약 19.5 GB", "LlavaOnevisionForConditionalGeneration.from_pretrained(..., torch_dtype=torch.bfloat16).cuda()"]
    ]
    add_styled_table(doc, app_a_headers, app_a_rows, [2.5, 4.2, 3.2, 1.8, 4.2])
    ...
    app_c_rows = [
        ...
        ["Depth-enhanced distance\ntime-series (GMH-D)", "Amprimo et al. (2024)", "3D 좌표 역투영 및 3x3 패치\n중앙값 필터링 방식 채택", "OptiTrack 120fps 광학 모션캡처 대비 검증된 마커리스 깊이 결합 손 추적 신뢰성을 본 연구 환경에 이식."],
    ...
    app_d_cases = [
        ("D1. Li 등 (2026, PLOS Digital Health) · 순수 영상 VLM의 시각적 환각과 본 연구의 출발점", [
            ...
            "■ 주요 결과: 일상 활동 분류에서는 77.5% 정확도를 보였으나, 미세 동작 정량화 및 FMA 손상 점수 예측에서는 완전히 실패함. 환자의 손이 물체에 스치기만 해도 파지로 오판하는 치명적 '시각적 접촉 환각'이 다수 관찰됨.",
            "■ 본 연구와의 연결: 본 연구의 주 비교(A2 − A1)를 도출한 직접적인 베이스 연구임. 2D 영상의 시각적 환각을 해결하기 위해 물리적 3D 센서 거리·시간 수치(F1~F4) 주입이 필수적임을 학술적으로 증명함."
        ]),
        ...
        ("D3. Amprimo 등 (2024, BSPC) · RGB-D 기반 3D 손 운동학 추출의 타당성", [
            "■ 연구 개요: Google MediaPipe 손 추적기에 Depth 센서(Azure Kinect / RealSense)를 결합한 GMH-D 시스템을 제안하고, 120 fps OptiTrack 광학 모션캡처와 동시 비교 검증함.",
            "■ 주요 결과: 손끝 거리 및 변위 시계열이 고가 모션캡처 장비와 매우 높은 상관성과 일치도(Bland-Altman)를 보임을 실증함.",
            "■ 본 연구와의 연결: 고가의 마커 기반 캡처 장비 없이도 RealSense D455와 MediaPipe 결합으로 임상적 3D 운동학 거리 시계열을 신뢰성 있게 추출할 수 있음을 입증한 기술적 닻(Anchor)임."
        ]),
    ...
    # Save to integrated file path
    out_integrated = Path(r"C:\Users\passp\OneDrive\바탕 화면\jeayong\capstone\연구계획서20260911_통합본.docx")
    try:
        doc.save(str(out_integrated))
        print(f"Successfully generated: {out_integrated}")
    except PermissionError:
        alt_path = out_integrated.parent / "연구계획서20260911_통합본_최신.docx"
        doc.save(str(alt_path))
        print(f"Word is currently holding a lock on {out_integrated.name}. Successfully saved updated version to: {alt_path}")

if __name__ == '__main__':
    create_integrated_proposal()
