"""[VLM FMA 평가 코드 2/2] LLaVA-OneVision-7B 기반 원통형 파지(Cylindrical Grasp) FMA 점수 측정
- 모델: llava-hf/llava-onevision-qwen2-7b-ov-hf (4-bit 양자화 적용)
...
def sample_and_annotate_frames(
    ...
    num_frames: int = 16,
    max_width: int = 384,
    save_preview_dir: str = "preview_frames_llavaonevision"
...
def run_llavaonevision_evaluation(video_path: str, output_md_path: str = "fma_result_llavaonevision_cylindrical.md"):
    print("=" * 72)
    print(" [2/2] LLaVA-OneVision-7B 모델 기반 원통형 파지(Cylinder) FMA 평가")
    print("=" * 72)
    ...
    video_arr, meta, duration_s = sample_and_annotate_frames(
        ...
        max_width=384,
        save_preview_dir="preview_frames_llavaonevision"
    )
    ...
    model = LlavaOnevisionForConditionalGeneration.from_pretrained(model_id, **load_kwargs)
    ...
    report_content = f"""# FMA-UE Cylindrical Grasp Assessment Report (LLaVA-OneVision-7B)
...
def main():
    parser = argparse.ArgumentParser(description="LLaVA-OneVision-7B Cylindrical Grasp FMA Evaluator")
    parser.add_argument("--video", type=str, default=DEFAULT_VIDEO_PATH, help="평가할 비디오 파일 경로 (.avi, .mp4)")
    parser.add_argument("--output", type=str, default="fma_result_llavaonevision_cylindrical.md", help="출력 마크다운 리포트 경로")
    args = parser.parse_args()

    run_llavaonevision_evaluation(args.video, args.output)
