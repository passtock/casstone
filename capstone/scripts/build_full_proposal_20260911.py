# -*- coding: utf-8 -*-
"""
Generate '연구계획서20260911.docx'
RGB-D 운동학 정보를 활용한 양손 손 과제 VLM 평가 파일럿 연구 계획서 개정안 (2026년 9월 11일)
Author: 한동대학교 기계제어공학 · Human Robotics Lab 22000561 이재용
Hardware Target: Single PC NVIDIA GeForce RTX 5090 (32GB GDDR7 VRAM)
Model Lineup:
  - 최신 모델군: Qwen3-VL-8B-Instruct, Qwen3-VL-30B-A3B-Instruct (AWQ 4-bit)
  - 선행연구 재현 모델군: LLaVA-NeXT-Video-7B, Qwen2.5-VL-32B-Instruct (AWQ 4-bit), LLaVA-OneVision-7B
  - 외부 API: 완전 배제 (100% 로컬 프라이빗 추론)
"""
...
def create_proposal():
    ...
    print("Building Document Structure with Updated Model Lineup...")

    # =========================================================================
    # DOCUMENT HEADER
    # =========================================================================
    p_title = add_para(doc, "RGB-D 운동학 정보를 활용한 양손 손 과제 VLM 평가 파일럿 연구", 
                       bold=True, size_pt=15.5, align=WD_ALIGN_PARAGRAPH.CENTER, 
                       space_before=6, space_after=4, color_rgb=(16, 44, 87))
    
    p_sub = add_para(doc, "연구계획서 개정안 (2026년 9월 11일)", 
    ...
    # =========================================================================
    # SECTION 0: 사전 개념 프레임워크 및 핵심 요약
    # =========================================================================
    add_h1(doc, "0. 연구 개요 및 사전 개념 체계 (Conceptual Framework)")
    
    add_para(doc, 
        "본 연구는 뇌졸중 환자의 재활 손 과제 영상에 센서 기반 3D 운동학(Kinematics) 정량 수치를 결합했을 때, "
        "시각-언어 모델(Vision-Language Model, VLM)의 기능 평가 판독 오차가 임상 전문가 기준 대비 유의미하게 감소하는지 실증하는 파일럿 연구이다. "
        "본 개정안에서는 외부 상용 API(Gemini 등) 의존성을 완전히 제거하고, 연구실 내 단일 PC(NVIDIA GeForce RTX 5090, 32GB VRAM) 환경에서 "
        "100% 로컬 오프라인으로 구동 가능한 '최신 고성능 모델 2종'과 '선행연구 기준 재현 모델 3종'으로 엄밀하게 모델군을 재편하였다."
    )

    add_callout(doc, "핵심 임상 배경 및 연구 가설의 요약 (Why this research?)", [
        "1. 임상적 문제 (Clinical Bottleneck): 뇌졸중 후 상지/손 기능 회복(FMA-UE 손 영역) 평가는 임상의의 직접적 육안 관찰과 수작업 채점에 의존하여 막대한 시간(비디오 주석에 사람 513시간 소요, PrimSeq 연구)과 평가자 간 변동성이 발생함.",
        ...
        "4. 로컬 완전 완결형 인프라 (100% On-Premise): 외부 API 전송에 따른 환자 의료영상 보안 이슈 및 비용 문제를 원천 차단하고, RTX 5090 (32GB VRAM) 1대에서 5개 모델 전체를 독립적으로 완벽 구동함."
    ])

    add_h2(doc, "0.1 실험 조건 사전 정의 및 명명 체계 (Nomenclature Framework)")
    add_para(doc, 
        "본 연구는 모델에게 제공되는 입력 정보 블록을 엄격히 통제하여, 어떤 정보가 평가 오차를 줄이는 핵심 요인인지 독립적으로 규명한다. "
        "이를 위해 아래와 같이 5개의 주요 조건(A1~A4, A0)과 피처 소거 조건(D1~D4)을 체계적으로 명명한다."
    )
    ...
    add_para(doc, 
        ...
        "만성 뇌졸중 환자의 6.4시간 분량 재활 비디오에서 세부 동작(도달, 파지, 유지 등)을 구분하고 주석을 다는 데 숙련된 평가자 513.6시간이 소요되었음을 보고하였다. "
    ...
    add_h2(doc, "4.2 물체 파지 과제 4대 핵심 운동학 피처")
    feat_headers = ["피처 ID 및 키", "변수명 및 공학적 조작 정의", "단위", "임상적 의미 및 선행연구 근거"]
    feat_rows = [
        ["F1\nacq_peak_span_mm", "동작 시작 신호 t0부터 유지 마커 th 사이 a(t)의 최댓값.\n(동률 시 최초 시점 사용, 보간점 제외)", "mm", "최대 파지 간격 (Peak Aperture). 물체 크기에 맞추어 손을 충분히 벌리는 파지 준비 능력 반영 (Qiu et al., 2022; Lang et al., 2005)."],
        ["F2\ntime_to_peak_s", "F1(최대 간격)이 발생한 시각에서 t0를 뺀 시간.\n(순수 초 단위로 보존, 분모 정규화 배제)", "s", "최대 간격 도달 시간 (Time to Peak Aperture). 도달과 파지 개방의 시간적 협응 능력 반영 (Qiu et al., 2022; Broome et al., 2019)."],
        ["F3\nhold_span_sd_mm", "유지 마커 th부터 th+2.0초까지 2초간 a(t)의 표본 표준편차.\n(유효 관측률 80% 이상에서만 산출)", "mm", "파지 유지 안정도 (Hold Stability SD). 물체 접촉 후 불수의적 떨림이나 파지력 상실 없이 형태를 고정하는 능력 (연구용 조작 정의)."],
    ...
    add_h2(doc, "4.3 맨손 쥐기/펴기 과제 4대 핵심 운동학 피처")
    ...
    free_rows = [
        ...
        ["F3\nhold_digit_sd_mm", "유지 신호 th 이후 2.0초 구간 동안 r(t)의 표본 표준편차.", "mm", "주먹 유지 안정도. 쥔 주먹 형태를 흔들림 없이 유지하는 능력."],
    ...
    # =========================================================================
    # SECTION 6: VLM 모델 구성, 프롬프트 및 단일 PC RTX 5090 환경 (UPDATED)
    # =========================================================================
    ...
    add_para(doc, 
        ...
        "모델군은 사용자의 요구에 맞추어 '최신 고성능 모델 2종'과 '선행연구 기준 재현 모델 3종'으로 엄격히 구성되었다. "
        "모든 모델은 14개 프레임 비전 토큰과 텍스트 프롬프트를 포함하여 32GB VRAM 내에서 완벽하게 구동됨을 사전 검증하였다."
    )
    
    vram_headers = ["모델 분류", "모델 식별자 (HF Model ID)", "가중치 정밀도", "가중치 VRAM", "비전+KV캐시", "총 VRAM 점유량", "32GB 안전성"]
    vram_rows = [
    ...
    add_h2(doc, "6.3 전문가 기준과 연구용 과제 수행등급 (0/1/2점) 정의")
    ...
    add_h2(doc, "6.4 시스템 프롬프트 및 구조화 JSON 출력 스키마")
    json_example = (
        "{\n"
        '  "left_hand": {\n'
    ...
    # Save Document
    output_path = Path(r"C:\Users\passp\OneDrive\바탕 화면\jeayong\capstone\연구계획서20260911.docx")
